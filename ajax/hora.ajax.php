...
$empleados=mysqli_query($link,"SELECT s.id AS 'sid', s.nombre AS 'snombre', s.duracion, e.descripcion, e.id AS 'eid', e.nombre AS 'enombre', e.status AS 'estatus', er.entrada, er.salida, er.lunes, er.martes, er.miercoles,  er.jueves, er.viernes,  er.sabado, er.domingo from(((servicios as s
left outer join servicios_empleados as ss on s.id=ss.servicios_id)
left outer join empleados as e on e.id=ss.empleados_id)
left outer join empleados_rol as er on er.empleados_id=e.id) WHERE servicios_id=".$obtener_id." AND ".$obtener_dia_letra."= 1");
...
function intervaloHora($hora_inicio, $hora_fin) {
    ...
    ==============================================================*/
    $rol_descanso = mysqli_query($link,"SELECT s.id AS 'sid', s.nombre AS 'snombre', s.duracion, e.id AS 'eid', e.nombre AS 'enombre', er.entrada AS 'entrada', er.salida AS 'salida',er.descanso AS 'descanso', er.duracion AS 'erduracion', er.lunes AS 'lunes', er.viernes AS 'viernes' FROM(((servicios as s
            left outer join servicios_empleados as ss on s.id=ss.servicios_id)
            left outer join empleados as e on e.id=ss.empleados_id)
            left outer join empleados_rol as er on er.empleados_id=e.id) WHERE e.status=1 AND servicios_id=".$obtener_id." AND ".$obtener_dia_letra."= 1");
    ...
    foreach ($rol_descanso as $key1oc =>  $empleado) {
        ...
        $est2=$res_estado;
        $contenido2 = $array[$i];
        $array_final[] = '<option '.$est1.$est2.'  value='.$contenido2.'>'.$contenido2.'</option><br>';
